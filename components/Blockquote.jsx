...
function BlockquoteWithImage({ author, image, children, className }) {
  return (
    ...
    >
      <blockquote className="col-span-2 text-xl/7 text-brand-espresso/70 sm:col-span-7 sm:col-start-6 sm:row-start-2">
        {typeof children === "string" ? <p>{children}</p> : children}
      </blockquote>
      <div className="col-start-1 row-start-2 overflow-hidden rounded-xl bg-brand-blush-light/40 sm:col-span-5 sm:row-span-full sm:rounded-3xl">
        <Image
          alt=""
          {...image}
      ...
      <figcaption className="text-sm text-brand-espresso sm:col-span-7 sm:row-start-3 sm:text-base">
  ...
  return (
    <Border position="left" className={clsx("pl-8", className)}>
      <figure className="text-sm">
        <blockquote className="text-brand-espresso/70 [&>*]:relative [&>:first-child]:before:absolute [&>:first-child]:before:right-full [&>:first-child]:before:content-['\u201c'] [&>:last-child]:after:content-['\u201d']">
          {typeof children === "string" ? <p>{children}</p> : children}
        </blockquote>
        <figcaption className="mt-6 font-semibold text-brand-espresso">
